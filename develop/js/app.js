...
function onGlobalClick(e) {
  ...
  const handlers = {
    ...
    deleteTeam: () => onDeleteTeam(el.dataset.teamId),
  ...
  return `
    <div class="glass-card mt-16">
      ...
      <div class="form-group hidden" id="addTeamDeptSection">
        ...
        <p class="text-dim mt-8" style="font-size:12px;">ここで入力した部署名が、出場メンバー登録時の部署選択の選択肢になります。</p>
        <div id="addTeamDeptList" class="mt-8">${deptRowHtml("", 0)}${deptRowHtml("", 1)}</div>
...
function deptRowHtml(value = "", idx = 0) {
  ...
  return `
    <div class="dept-row">
      <input class="field dept-name" placeholder="${placeholder}" maxlength="20" value="${escapeHtml(value)}">
    ...
    return `
      <div class="pending-row admin-team-row">
        ...
        <span class="admin-team-row-actions">
          ...
          <button type="button" class="icon-btn-sm icon-btn-sm-danger" data-action="deleteTeam" data-team-id="${tm.team_id}" aria-label="削除" title="削除">✕</button>
        </span>
      </div>`;
  }
