  ...
  return `
    <div class="glass-card mt-16">
      ...
      <div class="form-group hidden" id="addTeamDeptSection">
        ...
        <div id="addTeamDeptList" class="mt-8">${deptRowHtml()}</div>
...
/** 合同チームの「元となる部署名」1件分の入力行（大会作成フォーム・編集フォーム共通） */
function deptRowHtml(value = "") {
  return `
    <div class="dept-row">
      <input class="field dept-name" placeholder="例：法人事務部A" maxlength="20" value="${escapeHtml(value)}">
      <button type="button" class="remove-btn" data-action="removeDeptRow">×</button>
    </div>`;
}
function addDeptRow(targetId) {
  document.getElementById(targetId).insertAdjacentHTML("beforeend", deptRowHtml());
}
...
function adminTeamRowHtml(tm) {
  ...
  const deptRows = (sourceDepartments.length ? sourceDepartments : [""]).map((d) => deptRowHtml(d)).join("");
  return `
