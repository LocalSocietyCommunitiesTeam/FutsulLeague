...
/**
 * ヘッダー右上の「更新」ボタン（ホーム画面以外）。今表示している画面のデータだけを再取得する。
 * どのAPIを呼び直すかは画面（ルート）ごとに異なるため、ここで振り分ける。
 */
...
/** ヘッダー右上ボタンのアイコン（歯車＝管理者メニュー／更新＝表示中データの再取得）。JSの文字列定数として保持し、renderHeaderで差し替える。 */
const GEAR_ICON_SVG = `
...
function onGlobalClick(e) {
  ...
  const handlers = {
    ...
    saveEditTeam: () => onSaveEditTeam(el.dataset.teamId),
    deleteTeam: () => onDeleteTeam(el.dataset.entryId, el.dataset.teamName),
    saveMatchTeams: () => onUpdateMatchTeams(el.dataset.matchId),
...
function onGlobalInput(e) {
  if (e.target.classList && e.target.classList.contains("dept-select")) {
    onDeptSelectChange(e.target);
  }
...
function renderEntry() {
  ...
  return `
    <div class="glass-card">
      ...
      <div class="team-select-slider mt-8">${teamChips}</div>

      <h3 class="mt-16">${escapeHtml(team.name)} の出場メンバー</h3>
      <p class="text-dim mt-8">${deptNote}</p>
      <div id="memberList" class="mt-8">${buildMemberRowsHtml(team, showDept, deptOptions)}</div>
...
/** 既存メンバーを引き継ぎつつ、最低5枠は常に表示する（メンバーはエントリ単位で管理するためentry_idで絞り込む） */
function buildMemberRowsHtml(team, showDept, deptOptions) {
  const existing = appState.users.filter((u) => u.entry_id === team.entry_id);
  const rowCount = Math.max(existing.length, 5);
  let html = "";
  for (let i = 0; i < rowCount; i++) {
    html += memberRowHtml(i + 1, existing[i] || {}, showDept, deptOptions);
  }
  return html;
}
 ...
 * 合同チームの部署欄は、登録済みの元部署名（deptOptions）があればプルダウン、
 * 無ければ（未設定の古いデータ等への後方互換として）テキスト入力にフォールバックする。
 */
function memberRowHtml(idx, prefill = {}, showDept = true, deptOptions = []) {
...
function renderRanking() {
  if (!appState.rankingsLoaded) {
    loadRankings();
    return loadingState("順位を読み込んでいます…");
  }
...
function individualRankingHtml(list) {
  ...
  return list.map((r) => `
    <div class="rank-row">
      ...
      <div class="rank-points">${r.points}<span class="rank-sub">点</span></div>
    </div>`).join("");
}
...
function archiveTeamRankingHtml(list) {
  ...
  return list.map((r) => `
    <div class="rank-row">
      ...
      <div class="rank-points">${r.avg_points.toFixed(2)}<span class="rank-sub">pt</span></div>
    </div>`).join("");
}
...
function renderAdminDashboard() {
  ...
  if (!appState.admin.tournamentsLoaded) {
    ...
    return adminShellHtml(loadingState("大会一覧を読み込んでいます…"));
  }
...
/** JSON配列文字列を安全にパースする（コート構成・合同チームの元部署名など、共通で使う）。不正/未設定時は空配列。 */
function parseJsonArray(raw) {
  ...
  try {
    ...
    // 旧形式（カンマ区切りの文字列など）は自動移行できないため、空配列にフォールバックする
  }
  return [];
}
...
/* ---------- ③ チーム登録（コート予約の後に行う） ---------- */
/**
 * 「既に年度内で使われたことがある部署から選ぶ」か「新しい部署として登録する」かを選べる、
 * ハイブリッドな部署名入力欄を生成する共通UI。
 * 単一部署チームの名前欄・合同チームの元部署名欄の両方で使う。
 * 既知の部署が1件もない場合（年度で最初のチーム登録時など）は、素のテキスト入力のみ表示する。
 */
function departmentPickerHtml(value, knownDepartments, placeholder, extraClass = "") {
  if (!knownDepartments.length) {
    return `<input class="field dept-text-input ${extraClass}" placeholder="${placeholder}" maxlength="20" value="${escapeHtml(value)}">`;
  }
  const isKnown = value && knownDepartments.includes(value);
  const options = knownDepartments.map((d) => `<option value="${escapeHtml(d)}" ${d === value ? "selected" : ""}>${escapeHtml(d)}</option>`).join("");
  return `
    <select class="field dept-select ${extraClass}">
      <option value="">既存の部署から選ぶ</option>
      ${options}
      <option value="__new__" ${!isKnown ? "selected" : ""}>＋ 新しい部署として登録</option>
    </select>
    <input class="field dept-text-input mt-8 ${isKnown ? "hidden" : ""} ${extraClass}" placeholder="${placeholder}" maxlength="20" value="${isKnown ? "" : escapeHtml(value)}">
  `;
}
/** departmentPickerHtml の入力結果を1つの文字列として読み出す（プルダウンで選択済みならその値、そうでなければテキスト欄の値） */
function readDepartmentPickerValue(container) {
  const select = container.querySelector(".dept-select");
  const text = container.querySelector(".dept-text-input");
  if (select && select.value && select.value !== "__new__") return select.value.trim();
  return text ? text.value.trim() : "";
}
/** プルダウンで「＋新しい部署として登録」を選んだときだけテキスト入力を表示する */
function onDeptSelectChange(select) {
  const container = select.closest(".dept-row") || select.closest(".form-group");
  const textInput = container ? container.querySelector(".dept-text-input") : null;
  if (!textInput) return;
  const showText = select.value === "__new__" || select.value === "";
...
function selectTeamRegMode(el) {
  ...
  const existingSection = document.getElementById("addTeamExistingSection");
  const isNew = el.dataset.value === "NEW";
  if (existingSection) existingSection.classList.toggle("hidden", isNew);
...
function removeDeptRow(btn) {
  ...
  if (list.querySelectorAll(".dept-row").length <= 1) return; // 最低1枠は残す
  btn.closest(".dept-row").remove();
}
/** 参加形態チップ（単一部署／合同チーム）の選択と、合同チーム時の部署名セクションの表示切替を同時に行う */
function selectTeamTypeChip(el) {
  selectChip(el);
  const targetId = el.dataset.target;
  if (!targetId) return;
  const section = document.getElementById(targetId);
  if (section) section.classList.toggle("hidden", el.dataset.value !== "JOINT");
}
...
function adminTeamRowHtml(tm) {
    ...
    if (appState.admin.editingTeamId !== tm.team_id) {
      ...
      <div class="pending-row admin-team-row">
        ...
        <span class="admin-team-row-actions">
          <button type="button" class="icon-btn-sm" data-action="startEditTeam" data-team-id="${tm.team_id}" aria-label="編集" title="編集">✎</button>
          <button type="button" class="icon-btn-sm icon-btn-sm-danger" data-action="deleteTeam" data-entry-id="${tm.entry_id}" data-team-name="${escapeHtml(tm.name)}" aria-label="削除" title="削除">✕</button>
        </span>
      </div>`;
  }
    ...
    <div class="admin-team-edit-form">
      <div class="form-group">
        <label class="field-label">チーム名（部署名）</label>
        <input class="field" id="editTeamName" value="${escapeHtml(tm.name)}" maxlength="30">
        <p class="text-dim mt-8" style="font-size:12px;">この部署名を変更すると、同じ部署が参加している他の大会（同年度の別ラウンド）の表示にも反映されます。</p>
      </div>
      <div class="form-group">
        <label class="field-label">参加形態</label>
        <div class="select-chip-group" id="editTeamTypeGroup">
          <button type="button" class="select-chip ${tm.type !== "JOINT" ? "selected" : ""}" data-action="selectTeamTypeChip" data-target="editTeamDeptSection">単一部署</button>
          <button type="button" class="select-chip ${tm.type === "JOINT" ? "selected" : ""}" data-action="selectTeamTypeChip" data-value="JOINT" data-target="editTeamDeptSection">合同チーム</button>
        </div>
      </div>
      <div class="form-group ${tm.type === "JOINT" ? "" : "hidden"}" id="editTeamDeptSection">
        <label class="field-label">元となる部署名</label>
        <div id="editTeamDeptList" class="mt-8">${deptRows}</div>
        <button type="button" class="btn btn-ghost mt-8" data-action="addDeptRow" data-target="editTeamDeptList">＋ 部署を追加</button>
      </div>
      <div class="flex gap-8 mt-8">
        <button type="button" class="btn btn-primary" data-action="saveEditTeam" data-team-id="${tm.team_id}">保存</button>
        <button type="button" class="btn btn-ghost" data-action="cancelEditTeam">キャンセル</button>
      </div>
    </div>`;
}
function startEditTeam(teamId) {
  appState.admin.editingTeamId = teamId;
  render();
}
...
function updateScheduleEstimate() {
  ...
  const minPerTeam = Math.floor(avgPerTeam);
  const maxPerTeam = Math.ceil(avgPerTeam);
  ...
  box.innerHTML = `
    ...
    ${perCourtRowsHtml}
    <div class="schedule-estimate-row schedule-estimate-highlight">
      <span>チームごとの試合数（見込み）</span>
      <strong>${minPerTeam === maxPerTeam ? `${minPerTeam}試合` : `${minPerTeam}〜${maxPerTeam}試合`}</strong>
    </div>
...
/**
 * 自動生成された試合1件ごとに、対戦チーム（ホーム／アウェイ）を選択式で変更できるリストを表示する。
 * 対戦表生成後に組み合わせの間違いに気づいた場合の修正手段。時間・コートは変更しない。
 */
function matchTeamEditListHtml(matches, teams) {
  if (!matches.length) return `<p class="text-dim mt-8">試合がありません。</p>`;
  const sorted = [...matches].sort((a, b) => (a.start_time > b.start_time ? 1 : a.start_time < b.start_time ? -1 : 0));
  const teamOptions = (selectedId) => teams.map((t) => `<option value="${t.team_id}" ${t.team_id === selectedId ? "selected" : ""}>${escapeHtml(t.name)}</option>`).join("");

  return `
    <div class="match-team-edit-list mt-8">
      ${sorted.map((m) => `
        <div class="match-team-edit-row" data-match-id="${m.match_id}">
          <div class="match-team-edit-meta">${formatTimeDisplay(m.start_time)}〜${formatTimeDisplay(m.end_time)}　${escapeHtml(m.court_name)}</div>
          <div class="match-team-edit-selects">
            <select class="field match-edit-home">${teamOptions(m.home_team_id)}</select>
            <span class="text-dim">×</span>
            <select class="field match-edit-away">${teamOptions(m.away_team_id)}</select>
          </div>
          <button type="button" class="btn btn-ghost mt-8" data-action="saveMatchTeams" data-match-id="${m.match_id}">この対戦カードに変更する</button>
        </div>
      `).join("")}
    </div>`;
}

/** 対戦カード変更の保存前に、変更前後の対戦カードを示す確認ダイアログを出す */
async function onUpdateMatchTeams(matchId) {
  const row = document.querySelector(`.match-team-edit-row[data-match-id="${matchId}"]`);
  if (!row) return;
  const homeTeamId = row.querySelector(".match-edit-home").value;
  const awayTeamId = row.querySelector(".match-edit-away").value;
  if (homeTeamId === awayTeamId) { showToast("ホームとアウェイに同じチームは選べません", "error"); return; }
  ...
  const confirmed = window.confirm(
    `対戦カードを変更します。よろしいですか？\n\n` +
    `【変更前】${teamName(m.home_team_id)} × ${teamName(m.away_team_id)}\n` +
    `【変更後】${teamName(homeTeamId)} × ${teamName(awayTeamId)}`
  );
  if (!confirmed) return;
